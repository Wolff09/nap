	...
	for graph in iterator.components(merged):
		...
		if graph.number_of_edges() == 0:
			...
			for key, attr in graph.node.items():
				...
				n.save()
			Partition.create(pid=pid, diameter=nx.diameter(graph), num_nodes=graph.number_of_nodes(), num_edges=graph.number_of_edges(), num_artists=artists, num_top_artists=num_top_artists, density=nx.density(graph))
